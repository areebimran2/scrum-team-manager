...
export function TicketView({ input, mode, inputMembers, projects }) {
    ...
    const TicketCard = ({ ticket, projects}) => {
        if (mode === 'dashboard') {
            ...
            let project = projects.find(project => project.pid === ticket.project);
            let show = (typeof(project) !== "undefined");
            let projectName;
            if (show){
            ...
            return (
                <div className={styles.DticketCard}>
                    ...
                    <p className={styles.descriptors}>{projectName}</p>
    ...
    return (
        <div className={styles.ticketList}>
            {tickets.map(ticket => (<TicketCard key={ticket.tid} ticket={ticket} projects={projectlist} />))}
        </div>
    );
}